...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    $stmt = $conn->prepare("SELECT KayttajaID, Nimi, Gmail, SalasanaHash, Profiilikuva, Rooli FROM Kayttajat WHERE Gmail = ?");
    ...
    if ($stmt->num_rows == 1) {
        ...
        $kayttajaID = null; $nimi = null; $hash = null; $gmail_db = null; $profiilikuva = null; $rooli = null;
        $stmt->bind_result($kayttajaID, $nimi, $gmail_db, $hash, $profiilikuva, $rooli);
        $stmt->fetch();
        ...
        if (password_verify($salasana, $hash)) {
            ...
            $_SESSION['Rooli'] = $rooli; // Tallennetaan rooli sessioon
